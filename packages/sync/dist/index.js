...
var OpLog = class {
  constructor(opts) {
    this.opts = opts;
    for (const op of opts.persisted ?? []) {
  ...
  versionVector() {
    ...
    for (const op of this.ops.values()) {
      if (!(op.deviceId in vv) || op.lamport > vv[op.deviceId]) vv[op.deviceId] = op.lamport;
    }
    return vv;
  }
  ...
  opsSince(peerVV) {
    const out = [];
    for (const op of this.ops.values()) {
      if (op.lamport > (peerVV[op.deviceId] ?? 0)) out.push(op);
    }
    return out.sort((a, b) => a.lamport - b.lamport);
  }
  /** Record a LOCAL change. Returns the new op (caller broadcasts it to peers). */
  record(entity, entityId, kind, fields) {
    const op = {
      opId: this.opts.uuid(),
      entity,
      entityId,
      kind,
      fields: kind === "put" ? fields : void 0,
      lamport: ++this.clock,
      deviceId: this.opts.deviceId,
      ts: this.opts.now()
    };
    this.ops.set(op.opId, op);
    this.opts.persist?.(op);
    this.rematerialize(entity, entityId);
    return op;
  }
  ...
  ingest(incoming) {
    ...
    for (const op of incoming) {
      if (this.ops.has(op.opId)) continue;
      this.ops.set(op.opId, op);
      if (op.lamport > this.clock) this.clock = op.lamport;
      this.opts.persist?.(op);
      accepted.push(op);
      touched.add(`${op.entity}\0${op.entityId}`);
    }
    for (const key of touched) {
      const [entity, entityId] = key.split("\0");
      this.rematerialize(entity, entityId);
    }
    return accepted;
  }
  /** Recompute the winning op for one record and push it to the materializer. */
  rematerialize(entity, entityId) {
    let winner;
    for (const op of this.ops.values()) {
      if (op.entity !== entity || op.entityId !== entityId) continue;
      if (!winner || wins(op, winner)) winner = op;
    }
    if (!winner) return;
    if (winner.kind === "delete") this.opts.materializer.remove(entity, entityId);
    else this.opts.materializer.put(entity, entityId, winner.fields ?? {});
  }

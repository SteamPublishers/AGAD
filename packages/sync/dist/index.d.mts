import { P as PairingStatus, a as DeviceInfo, c as PairChallengeMessage, d as PairConfirmMessage, e as PairRejectMessage, f as PairRequestMessage, g as PairResponseMessage, h as PairedDevice, M as Message, T as TransferProgress, i as TextMessage, F as FileAcceptMessage, j as FileAckMessage, k as FileChunkMessage, l as FileCompleteMessage, m as FileRejectMessage, n as FileRequestMessage, o as FileTransfer, p as TextTransfer, D as DiscoveryService, b as DiscoveredDevice } from './index-D7PLqM1E.mjs';
export { A as AppMessage, q as AppSettings, C as ConnectionState, r as ConnectionStatus, s as DevicePlatform, E as ErrorMessage, t as FilesTransfer, H as HelloMessage, u as MDNS_DOMAIN, v as MDNS_SERVICE_NAME, w as MDNS_SERVICE_TYPE, x as MessageType, y as PairingChallenge, z as PairingResponse, B as PairingStep, G as PingMessage, I as PongMessage, S as StoredData, J as TXT_DEVICE_ID, K as TXT_DEVICE_NAME, L as TXT_PLATFORM, N as TXT_VERSION, O as Transfer, Q as TransferMetadata, R as TransferQueueItem, U as TransferType, V as createDiscoveredDevice, W as createTxtRecord, X as filterStaleDevices, Y as isDeviceStale, Z as parseTxtRecord, _ as removeDevice, $ as updateDeviceList } from './index-D7PLqM1E.mjs';
...
interface Op {
    /** Globally-unique op id (uuid). The dedup key across devices. */
    opId: string;
    /** Logical record type, e.g. 'conversation' | 'message' | 'project'. */
    entity: string;
    /** Stable id of the record this op mutates (must be a UUID, not autoincrement). */
    entityId: string;
    kind: OpKind;
    /** Full record fields for a 'put' (whole-record LWW). Omitted for 'delete'. */
    fields?: Record<string, unknown>;
    /** Lamport logical clock. */
    lamport: number;
    /** Origin device id. */
    deviceId: string;
    /** Wall-clock ms (display / human tiebreak only — never used for ordering). */
    ts: number;
}
/** Version vector: per-device highest lamport seen. */
type VersionVector = Record<string, number>;
/** How the op-log writes materialized state into the host's real store. */
...
interface OpLogOptions {
    deviceId: string;
    materializer: Materializer;
    /** Persist a single newly-accepted op (e.g. INSERT into sync_ops). */
    persist?: (op: Op) => void;
    /** Generate a uuid (host injects: crypto.randomUUID on Node, a polyfill on RN). */
    uuid: () => string;
    /** Wall clock ms. Injected so the core stays free of Date.now (testable). */
    now: () => number;
    /** Ops already on disk, to rehydrate the log at startup. */
    persisted?: Op[];
}
declare class OpLog {
    private readonly opts;
    ...
    /** Per-device highest lamport — what we tell a peer we already have. */
    versionVector(): VersionVector;
    /** Ops the peer (described by their version vector) hasn't seen yet. */
    opsSince(peerVV: VersionVector): Op[];
    /** Record a LOCAL change. Returns the new op (caller broadcasts it to peers). */
    record(entity: string, entityId: string, kind: OpKind, fields?: Record<string, unknown>): Op;
    /** Merge REMOTE ops. Returns those newly accepted (unseen), for chaining. */
    ingest(incoming: Op[]): Op[];
    /** Recompute the winning op for one record and push it to the materializer. */
    private rematerialize;
    /** Total ops held (diagnostics). */
    size(): number;
}

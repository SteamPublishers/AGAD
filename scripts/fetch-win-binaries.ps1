...
# scripts/llama-ref.txt) so grammar / native tool-call handling is byte-for-byte
...
#                     can't even load.
# The version has ONE owner: scripts/llama-ref.txt, shared with build-llama.sh. Hardcoding it in both is
# how the macOS build and the Windows binaries drift apart within a single release.
$LlamaRefFile = Join-Path $PSScriptRoot 'llama-ref.txt'
$LlamaRef = if ($env:LLAMA_REF) { $env:LLAMA_REF } else { (Get-Content $LlamaRefFile -Raw).Trim() }
